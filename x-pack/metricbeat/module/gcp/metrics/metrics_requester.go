...
func (r *metricsRequester) getFilterForMetric(serviceName, m string) string {
	...
	if serviceType, ok := gcp.GetGCPServicesType()[serviceName]; ok {
		switch serviceType {
		case gcp.ServiceRegional, gcp.ServiceZonal:
			...
			if serviceLabel, ok := gcp.GetServiceLabel()[serviceName]; ok {
				switch {
				case r.config.Region != "":
					f = fmt.Sprintf("%s AND %s = starts_with(\"%s\")", f, serviceLabel, strings.TrimSuffix(r.config.Region, "*"))
				case r.config.Zone != "":
					f = fmt.Sprintf("%s AND %s = starts_with(\"%s\")", f, serviceLabel, strings.TrimSuffix(r.config.Zone, "*"))
